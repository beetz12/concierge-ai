...
// For callback to backend
const BACKEND_URL = process.env.BACKEND_URL || 'http://localhost:8000';

/**
 * Save call result to database via backend API
 * This ensures the providers table is updated with call_status
 */
async function saveResultToDatabase(result, providerId, serviceRequestId) {
    if (!providerId || !serviceRequestId) {
        console.log('[Callback] Skipping database save - missing providerId or serviceRequestId');
        return;
    }

    try {
        console.log(`[Callback] Saving result to ${BACKEND_URL}/api/v1/providers/save-call-result`);
        const response = await fetch(`${BACKEND_URL}/api/v1/providers/save-call-result`, {
            method: 'POST',
        ...
        if (response.ok) {
            console.log(`[Callback] Successfully saved to database (status: ${response.status})`);
        } else {
            const errorText = await response.text();
            console.error(`[Callback] Failed to save to database: ${response.status} - ${errorText}`);
        }
    } catch (saveError) {
        console.error(`[Callback] Error saving to database: ${saveError.message}`);
    }
}
...
async function main() {
    ...
    try {
        ...
        while (['queued', 'ringing', 'in-progress'].includes(status) && attempts < maxAttempts) {
            ...
            try {
                ...
                if (status === 'ended') {
                    ...
                    // Save to database via backend callback
                    await saveResultToDatabase(result, PROVIDER_ID, SERVICE_REQUEST_ID);
                    return;
                }
            } catch (pollError) {
        ...
        // Save timeout result to database via backend callback
        await saveResultToDatabase(timeoutResult, PROVIDER_ID, SERVICE_REQUEST_ID);
        ...
        // Save error result to database via backend callback
        await saveResultToDatabase(errorResult, PROVIDER_ID, SERVICE_REQUEST_ID);
        process.exit(1);
    }
}
