...
### Architecture: Webhook + Database Storage

**Flow:**
1. VAPI initiates call via existing `call-provider.js` script
2. VAPI sends webhook to our API endpoint when call completes
3. API validates webhook, stores results in database
4. Kestra workflow polls database for results (lightweight query vs API call)

**Why this approach:**
- ✅ Real-time results (no polling delay)
- ✅ Reliable storage (survives restarts)
...
apps/api/src/
├── routes/
│   └── vapi.ts              # New file: VAPI webhook routes
├── services/
│   └── vapi.ts              # New file: VAPI business logic
└── lib/
    └── vapi-signature.ts    # New file: Webhook signature validation
```
...
export default async function vapiRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/webhooks/vapi', {
    ...
    try {
      ...
      if (!validation.success) {
        fastify.log.warn({ errors: validation.error.errors }, 'Invalid VAPI webhook payload');
   ...
   */
  fastify.get('/calls/by-phone/:phone', {
    schema: {
...
| GET | `/api/v1/vapi/calls/:callId` | Get call result by ID |
| GET | `/api/v1/vapi/calls/by-phone/:phone` | Get call history by phone |
