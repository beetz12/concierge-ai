...
import { LayoutDashboard, PlusCircle, PhoneCall, History, Settings, Menu, X, Info } from 'lucide-react';
...
const Sidebar: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-abyss text-slate-100 font-sans overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-surface text-slate-100 shadow-xl border-r border-surface-highlight">
        ...
        <nav className="flex-1 p-4 space-y-2">
          {navItems.map((item) => (
            ...
            >
              <item.icon className={`w-5 h-5 ${isActive(item.href) ? 'text-primary-400' : 'text-slate-400 group-hover:text-primary-400 transition-colors'}`} />
      ...
      {/* Mobile Sidebar */}
      <div className={`fixed inset-y-0 left-0 w-64 bg-surface text-slate-100 z-50 transform transition-transform duration-300 md:hidden border-r border-surface-highlight ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
