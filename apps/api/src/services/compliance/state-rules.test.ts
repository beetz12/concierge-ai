...
test("matrix integrity: covers all 50 states + DC (51 rows)", () => {
...
test("matrix integrity: exactly the matrix's 13 all-party states, incl. disputed CT/NV/DE/MI", () => {
...
test("matrix integrity: Oregon is one-party for telephone despite two-party lists", () => {
  assert.equal(STATE_RULES.OR.recordingConsent, "one_party");
});

test("matrix integrity: only Florida deviates from the federal quiet-hours window (8a-8p)", () => {
...
test("matrix integrity: only Washington carries the ADAD solicitation flat ban", () => {
...
test("matrix integrity: CA/UT/CO independently mandate AI disclosure", () => {
...
test("task-type taxonomy (R-27): every task type declares both solicitation flag and consent tier", () => {
...
test("task-type taxonomy (R-27): solicitation types require written consent; unknown fails safe", () => {
