 ...
 *
 * Tests are intentionally flat (no describe blocks) so each case reports as
 * its own top-level TAP test point.
 */
...
test("allow path: clean buyer-side daytime call is allowed", () => {
...
test("allow path: decision always carries the policy version and redial delegation", () => {
...
test("quiet hours (R-7): 8:59 p.m. callee-local is allowed", () => {
...
test("quiet hours (R-7): 9:01 p.m. callee-local is denied", () => {
  const decision = evaluate(
...
test("quiet hours (R-7): 9:00 p.m. exactly is outside the window", () => {
  const decision = evaluate(
...
test("quiet hours (R-7): 7:59 a.m. callee-local is denied, 8:00 a.m. is allowed", () => {
...
test("quiet hours (R-7): evaluated in the CALLEE's zone, not the platform's", () => {
...
test("quiet hours (R-7): Florida delta 7:59 p.m. allowed, 8:01 p.m. denied (8a-8p)", () => {
...
test("quiet hours (R-7): 8:30 p.m. is fine federally but past the Florida window", () => {
...
test("quiet hours (R-7): unknown area code fails closed — window must hold in every US zone", () => {
  // +1999... has no NANP region. At 18:30Z it is 8:30 a.m. in Hawaii (inside)
  // and 2:30 p.m. Eastern (inside) — allowed; at 15:00Z Hawaii is 5:00 a.m. —
  // denied even though Eastern is fine.
...
test("quiet hours (R-7): empty callee-local-times input fails closed", () => {
  const decision = evaluate(baseContext({ calleeLocalTimes: [] }));
  assert.ok(decision.reasons.includes("quiet_hours"));
});

test("recording mode (matrix): one-party state records one-party disclosed", () => {
  assert.equal(modeFor("+12125550100", "NY"), "one_party_disclosed");
  assert.equal(modeFor("+18645550100", "SC"), "one_party_disclosed");
});

test("recording mode (matrix): firm all-party states force announced all-party recording", () => {
...
test("recording mode (matrix): disputed states (CT/NV/DE/MI) fail closed to all-party", () => {
  for (const [number, state] of [
    ["+12035550100", "CT"],
    ["+17025550100", "NV"],
    ["+13025550100", "DE"],
...
test("recording mode (matrix): unknown state fails closed to all-party (R-6)", () => {
  assert.equal(modeFor("+19995550100", null), "all_party_disclosed");
});

test("recording mode (matrix): all-party mode adds the consent-flavored recording line", () => {
...
test("disclosures (R-12): every call gets identity, AI, and recording lines in order", () => {
...
test("disclosures (R-12): AI disclosure is present even where no state mandates it", () => {
...
test("disclosures (R-12): callback number is woven into the identity line when present", () => {
  const decision = evaluate(baseContext({ callbackNumber: "+18005550199" }));
  assert.ok(decision.disclosureLines[0]!.includes("+18005550199"));
});

test("disclosures (R-12): solicitation task types add the interactive opt-out line", () => {
  const decision = evaluate(
    baseContext({
...
test("gates: independent failures accumulate as distinct reasons", () => {
  const decision = evaluate(
    baseContext({
      userApproved: false,
...
test("solicitation (R-3/R-8/R-27): WA flat ban denies AI-voice solicitation regardless of consent", () => {
  const decision = evaluate(
    baseContext({
...
test("solicitation (R-3/R-8/R-27): WA buyer-side (non-solicitation) calls are not ADAD-banned", () => {
  const decision = evaluate(
...
test("solicitation (R-3/R-8/R-27): solicitation without written consent denies", () => {
...
test("solicitation (R-3/R-8/R-27): solicitation with written consent passes the consent gate", () => {
  const decision = evaluate(
    baseContext({
      taskType: "outbound_sales",
      recipientConsentTier: "prior_express_written",
...
test("solicitation (R-3/R-8/R-27): unknown task type fails safe to solicitation + written consent", () => {
...
test("solicitation (R-3/R-8/R-27): all buyer-side task types are non-solicitation and allowed", () => {
...
test("end-to-end: a Seattle number resolves to WA + Pacific and evaluates accordingly", () => {
