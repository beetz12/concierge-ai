...
describe("timezone: area code → state + IANA zone", () => {
  test("maps well-known area codes to their state and zone", () => {
  ...
  test("split-zone states carry per-area-code overrides", () => {
  ...
  test("unknown area codes and non-US numbers resolve to null", () => {
...
describe("timezone: callee-local time resolution", () => {
  test("computes wall-clock time in the callee's zone (DST-aware)", () => {
  ...
  test("known number yields exactly one local time", () => {
  ...
  test("unknown number yields every candidate US zone (fail closed, R-6)", () => {
    const times = resolveCalleeLocalTimes(
      "+19995550100",
      new Date("2026-07-03T16:30:00.000Z"),
    );
