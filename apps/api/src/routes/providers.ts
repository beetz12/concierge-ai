...
export default async function providerRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/call', {
    schema: {
      ...
      description: 'Makes a real phone call via VAPI.ai to gather availability and pricing information from a service provider. Automatically uses Kestra orchestration when available, falls back to direct VAPI calls otherwise.',
      body: {
        type: 'object',
  ...
  fastify.get('/call/status', {
    schema: {
      ...
      description: 'Returns information about the current calling method (Kestra or direct VAPI) and system health',
      response: {
        200: {
          type: 'object',
          properties: {
