...
import { VAPIApiClient } from '../services/vapi/vapi-api.client.js';
import type { CallResult, StructuredCallData } from '../services/vapi/types.js';
...
export default async function vapiWebhookRoutes(fastify: FastifyInstance) {
  ...
  if (!vapiApiClient && process.env.VAPI_API_KEY) {
    ...
    fastify.log.info({}, 'VAPI API client initialized for background enrichment');
  }
  ...
  fastify.post('/webhook', {
    schema: {
      ...
      description: 'Endpoint for VAPI.ai to send webhook events when calls complete. Stores results in cache for retrieval.',
      body: {
        type: 'object',
        properties: {
          message: {
    ...
    try {
      ...
      if (message.type !== 'end-of-call-report' && message.type !== 'call-end') {
        return reply.send({
          success: true,
      ...
      if (vapiApiClient) {
        triggerBackgroundEnrichment(call.id, fastify.log).catch(err => {
          fastify.log.error({ error: err, callId: call.id }, 'Background enrichment failed');
        });
      } else {
        fastify.log.warn({ callId: call.id }, 'VAPI API client not configured, skipping enrichment');
      ...
      return reply.send({
        success: true,
        message: 'Webhook processed and cached, background enrichment triggered',
        callId: call.id
      });
    } catch (error: unknown) {
   ...
   */
  fastify.get<{ Params: { callId: string } }>('/calls/:callId', {
    schema: {
      tags: ['vapi'],
      summary: 'Retrieve cached call result',
      description: 'Get call result from cache by call ID. Used by Kestra scripts to poll for results.',
      params: {
        type: 'object',
        properties: {
          callId: {
  ...
  fastify.get('/cache/stats', {
    schema: {
      ...
      description: 'Returns statistics about the webhook cache including size and entries (debugging only)',
      response: {
        200: {
          type: 'object',
          properties: {
   ...
   */
  fastify.delete<{ Params: { callId: string } }>('/calls/:callId', {
    schema: {
      tags: ['vapi'],
 ...
 */
async function triggerBackgroundEnrichment(callId: string, logger: FastifyBaseLogger): Promise<void> {
  ...
  for (let attempt = 0; attempt < MAX_FETCH_ATTEMPTS; attempt++) {
    try {
      ...
      const delay = FETCH_DELAYS_MS[attempt] || FETCH_DELAYS_MS[FETCH_DELAYS_MS.length - 1];
      logger.info({ callId, attempt: attempt + 1, delayMs: delay }, 'Waiting before VAPI API fetch');
      await new Promise(resolve => setTimeout(resolve, delay));

      // Fetch from VAPI REST API
      logger.info({ callId, attempt: attempt + 1 }, 'Fetching call data from VAPI API');
      ...
      if (vapiApiClient.isDataComplete(vapiCall)) {
        ...
        if (existingResult) {
          const enrichedData = vapiApiClient.mergeCallData(existingResult, vapiCall);
          ...
          logger.info({ callId }, 'Created fresh call result from VAPI API (cache was empty)');
        }
        return; // Success!
      }

      logger.warn({ callId, attempt: attempt + 1 }, 'VAPI API returned incomplete data, will retry');
    } catch (error: unknown) {
      logger.error({
        callId,
        attempt: attempt + 1,
  ...
  logger.error({ callId }, 'Max enrichment attempts reached, keeping partial data');
  webhookCache.updateFetchStatus(callId, 'fetch_failed', 'Max attempts reached');
}

/**
 * Transform VAPI webhook call data into CallResult format
 */
function transformVapiWebhookToCallResult(call: z.infer<typeof vapiWebhookCallSchema>): CallResult {
  ...
  const providerPhone = call.customer?.number || call.phoneNumber?.number || 'unknown';
